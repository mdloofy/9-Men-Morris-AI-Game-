...
def static_estimation(board):    #Calculates the static estimation for the given board position.
    ...
    old_estimation_score = num_white_pieces - num_black_pieces
    new_estimation_score = formationEvaluation(board)
    return old_estimation_score + new_estimation_score


def formationEvaluation(board):    # Define the formation evaluation function
    score = 0
    # Check for potential mills (three pieces in a row)

    for i in range(0, 21, 3):   # Rows
...
def minimax(board, depth, maximizing_player):  # Define the MINIMAX search function
    ...
    # Implement your own game-over condition based on the specific game rules
    if depth == 0:
...
# Function to determine the best move for White
