...
def build_game_tree(board, depth, maximizing_player):
    node = Node(board)
    ...
    moves = generateMovesBlack(board)
    for move in moves:
        temp = board
        board = move 
        child_node = build_game_tree(board, depth - 1, not maximizing_player)
        board = temp 
...
def minimax(node, maximizing_player):
    ...
    if maximizing_player:
        ...
        for child in node.children:
            eval_score = minimax(child, False)
            max_eval = max(max_eval, eval_score)
        return max_eval
    else:
        min_eval = float('inf')
        for child in node.children:
            eval_score = minimax(child, True)
            min_eval = min(min_eval, eval_score)
        return min_eval

def find_best_move(board, depth):
    game_tree = build_game_tree(board, depth, True)
...
def generateMovesBlack(board):   #generates all the moves possible for black
    temp_board = swapColors(board)
    reachable_positions = generateMovesMidgameEndgame(temp_board)
    return [swapColors(position) for position in reachable_positions]

def swapColors(board):     # swaps B with W in the board 
    swapped_board = []
    for piece in board:
        if piece == 'W':
